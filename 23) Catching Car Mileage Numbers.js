...
function isInteresting(number, awesomePhrases) {
  ...
  function checkInteresting(number,awesomePhrases){
  let arrNumber = number.toString().split('');
  if(number>=100){
    const upNumber = (number)=> {
    let temp = true;
    for(let i=0; i<arrNumber.length-1; i++){
      let x= (arrNumber[i]==9) ? 0 : Number(arrNumber[i])+1;
      if(x!=arrNumber[i+1]){return false}
    }
    return temp;
  }
    const downNumber = (number)=> {
    let temp = true;
    for(let i=0; i<arrNumber.length-1; i++){
      let x= Number(arrNumber[i])-1;
      if(x!=arrNumber[i+1]){return false}
    }
    return temp;
  }
    const zeroDigits = (number)=>{
    let temp = true;
  ...
  return temp;
    ...
    let temp = arrNumber.map((e,i)=> arrNumber[arrNumber.length-1-i]).join('');
    if(arrNumber.join('')== temp){return true}
    else{return false}
  }
    const awesome = (awesomePhrases.includes(number)) ? true : false ;
    let operation = [upNumber(),downNumber(),zeroDigits(),sameNumber(),palindrome(),awesome];
    if(operation.includes(true)){return true}
  }}
  if(checkInteresting(number,awesomePhrases)) return 2;
  if(checkInteresting(number+1,awesomePhrases) || checkInteresting(number+2,awesomePhrases)) return 1;
  return 0;
}
